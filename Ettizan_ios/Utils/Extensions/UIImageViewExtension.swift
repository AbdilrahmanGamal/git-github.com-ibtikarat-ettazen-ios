...
extension UIImageView {
    ...
    func setImageFromURL(urlString : String){
        ...
        print("************************************************************")
        print("SetImg ---> \(urlString)")
        print("************************************************************")

 
        var activityLoader = UIActivityIndicatorView()
        activityLoader = UIActivityIndicatorView(style: .gray)
        activityLoader.center = CGPoint(x: self.bounds.size.width/2, y: self.bounds.size.height/2)
        ...
        self.sd_setImage(with: url) { (image, error, cache, urls) in
            ...
            if (error != nil) {
                ...
                self.image = UIImage(named: "btn-cncl")
            } else {
                activityLoader.stopAnimating()
                self.image = image
            }
...
extension UIButton {
    func setimg(urlString : String){
        ...
        self.sd_setImage(with: url, for: .normal) { (image, error, cache, urls) in
            if (error != nil) {
                activityLoader.stopAnimating()
                //self.image = UIImage(named: "btn-cncl")
                self.setImage(UIImage(named: "btn-cncl"), for: .normal)
            } else {
                activityLoader.stopAnimating()
...
extension UIImage {
    ...
    func imageStr(str : String) -> UIImage?{
